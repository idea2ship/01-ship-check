...
- TypeScript
- Tailwind CSS
- Groq (`groq-sdk`, server-only) — Llama 3.3 70B
...
| 키 | 설명 |
| --- | --- |
| `GROQ_API_KEY` | https://console.groq.com 에서 발급한 Groq API 키 |
| `GROQ_MODEL` | (선택) 사용할 모델. 기본값 `llama-3.3-70b-versatile`. 후보: `llama-3.1-8b-instant`, `mixtral-8x7b-32768`, `gemma2-9b-it` |
...
├── lib/
│   ├── llm.ts                  # Groq client (parse / evaluate)
│   ├── supabase.ts
│   ├── markdown.ts
│   ├── validation.ts
│   └── types.ts
...
| Route | Method | 역할 |
| --- | --- | --- |
| `/api/parse` | POST | 자연어 아이디어 → `{ actor, situation, problem, solution, isComplete, ... }` (frontend는 900ms debounce 후 호출, Groq llama-3.3-70b 사용) |
| `/api/evaluate` | POST | 평가 결과 JSON 반환 (Groq llama-3.3-70b). `parsedIdea`를 함께 보내면 평가 안정성이 올라감 |
